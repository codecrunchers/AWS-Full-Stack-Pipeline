resource "aws_ecs_task_definition" "sonar_pipeline" {
  family                = "${format("%s_%s_family", var.stack_details["env"],lookup(var.pipeline_definition,"name"))}"
  container_definitions = "${data.template_file.sonar_task_definition_file.rendered}"
}

data "template_file" "sonar_task_definition_file" {
